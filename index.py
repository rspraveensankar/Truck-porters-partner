...
import re #pattern match for phno email
import time #timeout logout
...
# Database connection
def connect_db():
    return sqlite3.connect('porters.db')
...
def add_route():
    ...
    if st.button("Submit Route"):
        if all([pickupcode, dropcode, pickup_location, dropoff_location, zipcodes]):
            conn = connect_db()
            c = conn.cursor()
            c.execute("""INSERT INTO route (pickupcode, dropcode, pickup_location, dropoff_location, zipcodes, driver_phno) 
                         VALUES (?, ?, ?, ?, ?, ?)""",
...
def search_routes():
    ...
    if st.button("Search"):
        conn = connect_db()
        c = conn.cursor()
        c.execute("SELECT * FROM route WHERE zipcodes LIKE ?", ('%' + zipcode + '%',))
        routes = c.fetchall()
        conn.close()

        if routes:
            for route in routes:
                st.write(f"Pickup Location: {route[2]} - Dropoff Location: {route[3]} - Covered Zipcodes: {route[4]}")
        else:
            st.warning("No routes available for this zipcode.")

# Check Parcel Status (For users)
def check_parcel_status():
    ...
    if st.button("Check Status"):
        ...
        if parcel:
            st.info(f"Parcel Status: {parcel[0]}")
        else:
            st.warning("Parcel ID not found!")

# Function to handle logout
def logout():
    st.session_state.role = None
    st.session_state.phno = None
    st.session_state.login_time = None
    st.success("Logged out successfully!")

# Main app logic with Logout option
def main():
    ...
    # If user or driver is logged in
    ...
    # If no one is logged in, show login and registration options
    else:
